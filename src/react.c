...
int React (realtype t, realtype stepsize, Chem_Data * CD, int cell, int * NR_times){
  ...
  double tmpval, tmpprb, sat_inv, I, Iroot, tmpKeq, ISupdateflg, adh, bdh, bdt, maxerror = 1, surf_ratio, tot_cec, tmpprb_inv;
  realtype ** jcb;

  control    = 0;
  tmpprb     = 1.0E-3;
  tmpprb_inv = 1.0 /tmpprb ;
  sat_inv    = 1.0 / CD->Vcele[cell].sat;
  ...
  for ( i = 0;  i<CD->NumStc; i++){
    if (CD->chemtype[i].itype == 1){
      ...
      // Note that the concentration of solid is defined differently than aqueous species in unsaturated zone. The former in mol/L pore volume and the latter in mol/L water. Why do we do this, the definition of surface area in aqueous unit in unsaturated zone is quite complicated.
      // Also note that the concentration of exhcange and adsorption sites are defined similarly as aqueus species although they should not. This is to preserve mass balance and facilitate equilibirum calculation. To translate them back to mol/L pore volume, a post processing code need to be written. Just multiply the concentration in the conc file by unsaturation zone equivalent depth and divide again by the total unsaturated zone depth. Note as we specified before, solid does not need this.
    }
  }
  ...
  for ( i = 0 ; i < CD->NumSpc; i ++)
    Rate_spe[i] = Rate_spe[i]/(CD->Vcele[cell].porosity * CD->Vcele[cell].sat);
  */
