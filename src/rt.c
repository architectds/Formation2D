...
void chem_alloc(char *, const void *, Chem_Data *, realtype);
  ...
  //    FILE*        logfile = fopen("logfile/pihmlog.out","w");
    ...
    *rawtime    = (int)(t*60);
    ...
    for ( j = 0 ; j < CD->NumEle; j ++){
      ...
      if (( hg * ht > 0)) partratio  = ht * 0.30 / hg;  // One forth indicate third order relationship between S and Kr, and so on
      if (  hg <= 0)      partratio  = 1.00E3;          // no groundwater and flow essentially
      if (  ht <= 0)      partratio  = 1.00E-3;         // no transient zone flow essentially
	...
	A = (tmpflux[CD->Flux[i].nodeup - 1] + tmpflux[CD->Flux[i].nodelo - 1]) * 0.5;
      else {
    ...
    for(i = 0 ; i < CD->NumEle; i++){
      sumflux1   = (CD->Vcele[i].height_t - CD->Vcele[i].height_o)*MD->Ele[i].area*MD->Ele[i].Porosity;
      ...
      CD->Flux[CD->Vcele[i].ErrDumper - MD->NumEle].flux      = - CD->Flux[CD->Vcele[i].ErrDumper].flux ;
      CD->Flux[CD->Vcele[i].ErrDumper - MD->NumEle].velocity  = - CD->Flux[CD->Vcele[i].ErrDumper].velocity;
    ...
    for ( i = CD->NumEle; i < CD->NumEle * 2 ; i ++){
      sumflux1   = (CD->Vcele[i].height_t - CD->Vcele[i].height_o) * MD->Ele[i - MD->NumEle].area * MD->Ele[i - MD->NumEle].Porosity;
      sumflux2   = sumflux1 - resflux[i];
      correction = - sumflux2 * UNIT_C / stepsize / CD->Vcele[i].q;
      A          = CD->Flux[CD->Vcele[i].ErrDumper].flux;
      CD->Vcele[i].q  = sumflux2 * UNIT_C / stepsize;
      CD->Vcele[i].q  = MAX(CD->Vcele[i].q, 0.0);   // input of rain water chemistry can not be negative;
      CD->Vcele[i].q += fabs(MD->EleET[i-MD->NumEle][2]) * MD->Ele[i - MD->NumEle].area;
...
void chem_alloc(char* filename, const void *DS, Chem_Data *CD, realtype t){

  int i, j, k, num_face =0, num_species, num_mineral, num_ads, num_cex, num_other, num_conditions = 0, line_width = LINE_WIDTH, 
    words_line = WORDS_LINE, word_width = WORD_WIDTH, Global_diff = 0, Global_disp = 0, error_flag = 0,
    speciation_flg = 0, specflg;
  ...
  char* forcfn   = (char*) malloc ((strlen(filename)+12)*sizeof(char));
  sprintf(forcfn, "input/%s.prep",filename);
  ...
  while (keymatch(line, "END", tmpval, tmpstr)!=1){
    ...
    if ( keymatch(line, "Mobile_exchange",tmpval,tmpstr) == 1){
      CD->TimRiv = tmpval[0];
      fprintf(stderr, " Ratio of mobile ion exchange site %f.\n", CD->TimRiv);
      // under construction.
    }
  }
  ...
  for ( i = 0; i < num_conditions + 1; i ++){ // all conditions + precipitation
  ...
  if ( CD->chemtype == NULL ) fprintf(stderr , " Memory allocation error\n" ); 
  ...
  while (keymatch(line, "INITIAL_CONDITIONS", tmpval, tmpstr)!=1)
    fgets(line, line_width, chemfile);
  if ( strcmp(tmpstr[1], "FILE" ) == 0 ){ // initialize chemical distribution from file evoked. This will nullify all the condition assignment given in the next lines. But for now, please keep those lines to let the code work.
    ...
    if ( cheminitfile == NULL) fprintf(stderr, " Chemical initial distribution file is missing from input directory!\n");
    initfile = 1;
  }
  fgets(line, line_width, chemfile);
  while (keymatch(line, "END", tmpval, tmpstr)!=1){
  ...
  if ( cheminitfile != NULL) fclose(cheminitfile);
  ...
  for ( i = 0; i < num_conditions; i++){
	...
	if((tmpstr[0][0] == '>')||( specflg == 2)){ // adsorptive sites and species start with >
	  Condition_vcele[i].t_conc[CD->NumSpc + num_ads] = tmpval[0];          // this is the site density of the adsorptive species.
  ...
  if ( CD->PrpFlg == 2){
    ...
    assert(CD->TSD_prepconc->index == CD->NumSpc);
  ...
  */
  ...
  for ( i = 0; i < MD->NumEle; i++){
    ...
    CD->Vcele[i].temperature = MD->Ele[i].temp;
  ...
  for ( i = 2*MD->NumEle; i < 2*MD->NumEle + MD->NumRiv ; i ++){
    ...
    CD->Vcele[i].area     = MD->Riv[j].Length * CS_AreaOrPerem(MD->Riv_Shape[MD->Riv[j].shape - 1].interpOrd,MD->DummyY[j + 3*MD->NumEle],MD->Riv[j].coeff,3);
  ...
  for ( i = 2*MD->NumEle + MD->NumRiv ; i < 2*MD->NumEle + 2*MD->NumRiv; i ++){
  ...
  for ( i = 0 ; i< MD->NumEle; i++){
	...
	CD->Flux[k].nodeuu      = upstream(MD->Ele[i], MD->Ele[MD->Ele[i].nabr[j]-1], MD);
	CD->Flux[k].nodell      = upstream(MD->Ele[MD->Ele[i].nabr[j]-1], MD->Ele[i], MD);
	CD->Flux[k].flux_type   = 0;
	CD->Flux[k].BC          = 0;
	if (CD->Flux[k].nodeuu  > 0)
	  CD->Flux[k].distuu    = sqrt(pow(MD->Ele[i].x - MD->Ele[CD->Flux[k].nodeuu-1].x,2) + pow(MD->Ele[i].y - MD->Ele[CD->Flux[k].nodeuu-1].y,2));
	else
	  CD->Flux[k].distuu    = 0.0;
	if (CD->Flux[k].nodell  > 0)
	  CD->Flux[k].distll    = sqrt(pow(MD->Ele[MD->Ele[i].nabr[j]-1].x - MD->Ele[CD->Flux[k].nodell-1].x,2) + pow(MD->Ele[MD->Ele[i].nabr[j]-1].y - MD->Ele[CD->Flux[k].nodell-1].y,2));
	else
	  CD->Flux[k].distll    = 0.0;
	  
	CD->Flux[k].distance    = sqrt(pow(MD->Ele[i].x - MD->Ele[MD->Ele[i].nabr[j]-1].x,2) + pow(MD->Ele[i].y - MD->Ele[MD->Ele[i].nabr[j]-1].y,2));
	...
	if ( (para_a * MD->Ele[i].x + para_b * MD->Ele[i].y + para_c ) * (para_a * MD->Ele[MD->Ele[i].nabr[j] - 1].x + para_b * MD->Ele[MD->Ele[i].nabr[j] - 1].y + para_c ) > 0.0)    fprintf(stderr, " two points at the same side of edge!\n");
	dist1 = fabs(para_a * MD->Ele[i].x + para_b * MD->Ele[i].y + para_c )/ sqrt (para_a * para_a + para_b * para_b);
	dist2 = fabs(para_a * MD->Ele[MD->Ele[i].nabr[j] - 1].x + para_b * MD->Ele[MD->Ele[i].nabr[j] - 1].y + para_c ) / sqrt(para_a * para_a + para_b * para_b);
	if (( CD->Flux[k].distance < dist1) || ( CD->Flux[k].distance < dist2)){
	  ...
	  fprintf(stderr, " Ele coordinates, x: %f, y: %f\n Node_1 coordinates, x: %f, y: %f\n Node_2 coordinates, x: %f, y: %f\n Nabr x:%f, y:%f\n", MD->Ele[i].x, MD->Ele[i].y, x_0, y_0, x_1, y_1, MD->Ele[MD->Ele[i].nabr[j] - 1].x, MD->Ele[MD->Ele[i].nabr[j] - 1].y);
	  fprintf(stderr, " node_1 x:%f y:%f\t node_2 x:%f y:%f node_3 x:%f y:%f nabr x:%f y:%f, no:%d\n",MD->Node[MD->Ele[i].node[0]-1].x, MD->Node[MD->Ele[i].node[0]-1].y, MD->Node[MD->Ele[i].node[1]-1].x, MD->Node[MD->Ele[i].node[1]-1].y, MD->Node[MD->Ele[i].node[2]-1].x, MD->Node[MD->Ele[i].node[2]-1].y, MD->Ele[MD->Ele[i].nabr[j] - 1].x, MD->Ele[MD->Ele[i].nabr[j]-1].y, j);
	  }
	else{
	  CD->Flux[k].distance = dist1;
	}
	for ( rivi = 0; rivi < CD->NumRiv; rivi ++){
	  if (( CD->Flux[k].nodeup == MD->Riv[rivi].LeftEle) && ( CD->Flux[k].nodelo == MD->Riv[rivi].RightEle)){
	    CD->Flux[k].nodelo = 2 * CD->NumEle + CD->NumRiv + rivi + 1;
	    CD->Flux[k].nodeuu = 0;
	    CD->Flux[k].nodell = 0;
	    CD->Flux[k].distance = dist1;
	    //	    fprintf(stderr, " Riv - Watershed connection corrected, now from cell %d to cell %d\n", CD->Flux[k].nodeup, CD->Flux[k].nodelo);
	  }
	  if (( CD->Flux[k].nodeup == MD->Riv[rivi].RightEle) && ( CD->Flux[k].nodelo == MD->Riv[rivi].LeftEle)){
	    CD->Flux[k].nodelo = 2 * CD->NumEle + CD->NumRiv + rivi + 1;
	    CD->Flux[k].nodeuu = 0;
	    CD->Flux[k].nodell = 0;
	    CD->Flux[k].distance = dist1;
  ...
  for ( i = 0 ; i< MD->NumEle; i++){
    for (j = 0 ; j < 3; j++)
      if ( MD->Ele[i].nabr[j] > 0){
        ...
        CD->Flux[k].nodeuu      = upstream(MD->Ele[i], MD->Ele[MD->Ele[i].nabr[j]-1], MD) + MD->NumEle;
        CD->Flux[k].nodell      = upstream(MD->Ele[MD->Ele[i].nabr[j]-1], MD->Ele[i], MD) + MD->NumEle;
        CD->Flux[k].flux_type   = 0;
        CD->Flux[k].BC          = 0;
        if (CD->Flux[k].nodeuu  > 0)
          CD->Flux[k].distuu    = sqrt(pow(MD->Ele[i].x - MD->Ele[CD->Flux[k].nodeuu-1].x,2) + pow(MD->Ele[i].y - MD->Ele[CD->Flux[k].nodeuu-1].y,2));
	...
	CD->Flux[k].distance = dist1;
        for ( rivi = 0; rivi < CD->NumRiv; rivi ++){
          if (( CD->Flux[k].nodeup - CD->NumEle == MD->Riv[rivi].LeftEle) && ( CD->Flux[k].nodelo - CD->NumEle == MD->Riv[rivi].RightEle)){
            CD->Flux[k].nodelo = 2 * CD->NumEle + CD->NumRiv + rivi + 1;
            CD->Flux[k].nodeuu = 0;
            CD->Flux[k].nodell = 0;
            CD->Flux[k].distance = dist1;
          }
          if (( CD->Flux[k].nodeup - CD->NumEle == MD->Riv[rivi].RightEle) && ( CD->Flux[k].nodelo - CD->NumEle  == MD->Riv[rivi].LeftEle)){
            CD->Flux[k].nodelo = 2 * CD->NumEle + CD->NumRiv + rivi + 1;
            CD->Flux[k].nodeuu = 0;
            CD->Flux[k].nodell = 0;
            CD->Flux[k].distance = dist1;
  ...
  */
  ...
  fprintf(stderr, " Mass action species type determination (0: immobile, 1: mobile, 2: Mixed)\n");
  for ( i = 0; i < CD->NumSpc; i ++){
  ...
  fprintf(stderr, " Individual species type determination (1: aqueous, 2: adsorption, 3: ion exchange, 4: solid)\n");
  for ( i = 0; i < CD->NumStc + CD->NumSsc; i ++){
    fprintf(stderr, " %12s\t%10d\n",CD->chemtype[i].ChemName, CD->chemtype[i].itype);
  }
  ...
  for ( i = 0 ; i < CD->NumVol; i ++){
	...
	CD->Vcele[i].t_conc[j] = CD->Vcele[i].t_conc[j] * ( 1 / CD->Vcele[i].porosity - 1) * 2650 ;
	CD->Vcele[i].p_conc[j] = CD->Vcele[i].t_conc[j];
...
void fluxtrans(realtype t, realtype stepsize, const void *DS, Chem_Data *CD, N_Vector  VY){
  ...
  *rawtime    = (int)(t*60);
  ...
  for ( i = MD->NumEle; i < 2*MD->NumEle; i++){
    j  = i - MD->NumEle;
    CD->Vcele[i].q        += MAX(MD->ElePrep[j], 0.0) * CD->Vcele[i].area;
  }
  ...
  */
  for ( i = 0 ; i< MD->NumEle; i++){
    CD->Vcele[i].height_tl              = Y[i + 2 * MD->NumEle];
    CD->Vcele[i + MD->NumEle].height_tl = Y[i + MD->NumEle];

    for (j = 0 ; j < 3; j++)
      if ( MD->Ele[i].nabr[j] > 0){
        /* node indicates the index of grid blocks, not nodes at corners */
	CD->Flux[k].flux     += MD->FluxSub[i][j];
	CD->Flux[k].s_area   += MD->AreaSub[i][j];
	CD->Flux[k].velocity += MD->VeloSub[i][j];
        k++;
      }
  ...
  for ( i = MD->NumEle; i< 2*MD->NumEle; i++){
    CD->Flux[k].velocity += MD->Recharge[i-MD->NumEle];
    CD->Flux[k].flux     += MD->Recharge[i-MD->NumEle] * CD->Vcele[i].area;
    CD->Flux[k].s_area   += CD->Vcele[i].area;
    k++;
  }

  for ( i = 0; i<MD->NumEle; i++){
    CD->Flux[k].velocity += -MD->Recharge[i];
    CD->Flux[k].flux     += -MD->Recharge[i] * CD->Vcele[i].area;
    CD->Flux[k].s_area   += CD->Vcele[i].area;
    k++;
  }
  ...
  */
  for ( i = 0; i <MD->NumRiv; i++){
    ...
    CD->Flux[k].flux   += MD->FluxRiv[i][7] + MD->FluxRiv[i][4];
    k++;
  }
  for ( i = 0; i <MD->NumRiv; i++){
    //CD->Flux[k].flux   += MD->FluxRiv[i][8];
    CD->Flux[k].flux   += MD->FluxRiv[i][8] + MD->FluxRiv[i][5];
    k++;
  }
  for ( i = 0; i <MD->NumRiv; i++){
    CD->Flux[k].flux   += MD->FluxRiv[i][9] + MD->FluxRiv[i][1];
    k++;
  }
  for ( i = 0; i <MD->NumRiv; i++){
    CD->Flux[k].flux   += MD->FluxRiv[i][10] +MD->FluxRiv[i][0];
    k++;
  }
  ...
  if ( ((int)timelps - (int)CD->TimLst) == CD->AvgScl * stepsize) {
	...
	CD->TSD_prepconc->iCounter ++;
      ...
      if ( CD->Precipitation.t_conc[0] != CD->TSD_prepconc->TS[CD->TSD_prepconc->iCounter][1])
	for (i = 0 ; i < CD->NumSpc; i ++){
	  CD->Precipitation.t_conc[i] = CD->TSD_prepconc->TS[CD->TSD_prepconc->iCounter][i+1];
	  fprintf(stderr, " %s in precipitation is changed to %6.4f\n", CD->chemtype[i].ChemName, CD->Precipitation.t_conc[i]);
	}
    }
    ...
    for ( i = 0; i < MD->NumEle; i++){
      ...
      CD->Vcele[i].height_t = MAX(Y[ i + 2 * MD->NumEle ] , 1.0E-5);
    ...
    for ( i = 2*MD->NumEle; i < 2*MD->NumEle + MD->NumRiv; i++){
      j = i - 2 * MD->NumEle;
      CD->Vcele[i].height_o = CD->Vcele[i].height_t;
      CD->Vcele[i].height_t = MAX(Y[i+MD->NumEle],1.0E-5);
      CD->Vcele[i].height_int = CD->Vcele[i].height_t;
      CD->Vcele[i].height_sp= (CD->Vcele[i].height_t - CD->Vcele[i].height_o) * invavg;
      CD->Vcele[i].area     = MD->Riv[j].Length * CS_AreaOrPerem(MD->Riv_Shape[MD->Riv[j].shape - 1].interpOrd,Y[j + 3*MD->NumEle],MD->Riv[j].coeff,3);
      CD->Vcele[i].vol_o    = CD->Vcele[i].area * CD->Vcele[i].height_o;
      CD->Vcele[i].vol      = CD->Vcele[i].area * CD->Vcele[i].height_t;
    }
    /* update EBR cells */
    for ( i = 2*MD->NumEle + MD->NumRiv; i < 2*( MD->NumEle + MD->NumRiv ) ; i++){
      j = i - 2 * MD->NumEle - MD->NumRiv;
      CD->Vcele[i].height_o = CD->Vcele[i].height_t;
      CD->Vcele[i].height_t = MAX(Y[i+MD->NumEle],1.0E-5) + MAX(Y[i+MD->NumEle - MD->NumRiv], 1.0E-5)/CD->Vcele[i].porosity;
      CD->Vcele[i].height_int = CD->Vcele[i].height_t;
      CD->Vcele[i].height_sp= (CD->Vcele[i].height_t - CD->Vcele[i].height_o) * invavg;
      CD->Vcele[i].area     = MD->Riv[j].Length * CS_AreaOrPerem(MD->Riv_Shape[MD->Riv[j].shape - 1].interpOrd,Y[j + 3*MD->NumEle],MD->Riv[j].coeff,3);
      CD->Vcele[i].vol_o    = CD->Vcele[i].area * CD->Vcele[i].height_o;
    ...
    */
    ...
    if ( CD->TimLst >= CD->Delay ) {
      ...
      AdptTime(CD, CD->TimLst, rt_step, stepsize * (double)CD->AvgScl, &rt_step, MD->NumEle * 2); 
	...
	for ( j = 0 ; j < CD->NumStc; j ++)
	  {
	    if ( CD->chemtype[j].itype == 4 )
	      {
		CD->Vcele[i].t_conc[j] = (CD->Vcele[i].t_conc[j] * CD->Vcele[i].height_t + CD->Vcele[i + CD->NumEle].t_conc[j]*( CD->Vcele[i].height_v - CD->Vcele[i].height_t) )/ CD->Vcele[i].height_v;
      ...
      for ( i = 0 ; i < CD->NumOsv; i ++){
	if( fabs(CD->Vcele[i].height_t - CD->Vcele[i].height_int ) > 1.0E-6) fprintf(stderr, "%d %6.4f\t%6.4f\n", i, CD->Vcele[i].height_t, CD->Vcele[i].height_int); 
...
void InitialChemFile(char* filename){
  ...
  *rawtime    = (int)(t*60);
	...
	for ( i = 0; i < CD->NumStc; i ++){
	  for ( j = CD->NumEle * 2; j < CD->NumOsv ; j++)
	    if (CD->chemtype[i].itype == 4)
	      ...
	      CD->Vcele[j].p_conc[i] = fabs(CD->Vcele[j].t_conc[i]*0.1);
	}
      }
    if ( ! CD->RecFlg)
  ...
  if ( (int)timelps%(CD->OutItv * 60) == 0){
	...
	fprintf(Cfile[1], "%6s\t", CD->chemtype[i].ChemName);
      for ( i = 0 ; i < CD->NumSsc; i++)
	fprintf(Cfile[1], "%6s\t", CD->chemtype[i+CD->NumStc].ChemName);
      fprintf(Cfile[1],"\n");
    }
    /* print the header of file if first time entered */
    fprintf(Cfile[1],"\"%4.4d-%2.2d-%2.2d %2.2d:%2.2d\"\t",timestamp->tm_year+1900,timestamp->tm_mon+1,timestamp->tm_mday,timestamp->tm_hour,timestamp->tm_min);
    for (j = 0; j < CD->NumStc; j++)
...
void AdptTime(Chem_Data * CD, realtype timelps, double rt_step, double hydro_step, double * start_step, int num_blocks)
{
  ...
  if ( rt_step >= hydro_step ) int_flg = 0;
  else                         int_flg = 1;
  ...
  while ( timelps < end_time){
    ...
    if ((!CD->RecFlg) && ((int)(timelps+stepsize) % (int)(CD->React_delay*stepsize) == 0)){
      for ( i = 0 ; i < num_blocks; i++){
	if (CD->Vcele[i].illness < 20)
	  if (React(timelps - (CD->React_delay*stepsize), stepsize * CD->React_delay, CD, i, &nr_tmp))
	    {
	      ...
	      if ( j == 0)
		{
		  ...
		  for ( j = 1; j < k ; j ++)
		    {
		      React(timelps + j * substep, substep, CD, i, &nr_tmp);
		      tot_nr += nr_tmp;
		    }
    ...
    timer2 = timer();
    timelps += stepsize;
    if ( timelps >= org_time + hydro_step) break;
  }
  //  ReportError(CD->Vcele[1075], CD);
  if ((!CD->RecFlg) && ((int)(timelps) % (int)(CD->React_delay*stepsize) == 0)){
    fprintf(stderr, "  React from %f to %f, Average newton iteration taken: %f, time elapsed %6.4f seconds\n",timelps - (CD->React_delay * stepsize), timelps, (double)tot_nr/(double)CD->NumEle/2.0, timer2-timer1);
  }
}
